...
| §13.1–§13.2 Desktop modules | Tray, window lifecycle, hotkeys, crash reports, portals | Done | `src-tauri/src/` — `tray.rs`, `main_window.rs` (opens at the default size, never maximized; `desktop.keepWindowInMemory`, `settings.rs:342`), `hotkeys.rs`, `crash_dump.rs`, `portals.rs`. |
