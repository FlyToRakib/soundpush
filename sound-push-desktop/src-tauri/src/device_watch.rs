    ...
    let spawned = std::thread::Builder::new()
        ...
        .spawn(move || {
            while let Ok(first) = rx.recv() {
                ...
                {
                    let _ =
                        engine.audio_devices_changed(change.default_input, change.default_output);
                }
            }
        });
        ...
        fn OnDefaultDeviceChanged(
            ...
            if role == eConsole {
                let _ = self.0.send(Change {
                    default_input: flow == eCapture,
                    default_output: flow == eRender,
                });
            }
            Ok(())
        }
        ...
        let spawned = std::thread::Builder::new()
            ...
            .spawn(move || {
                ...
                match registered {
                    ...
                    Err(e) => {
                        warn!(error = %e, "could not register for audio device notifications")
                    }
                }
            });
        if let Err(e) = spawned {
