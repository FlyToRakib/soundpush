...
pub const OPUS_BITRATE_STEPS: &[u32] = &[
    10_000, 16_000, 24_000, 32_000, 48_000, 64_000, 96_000, 128_000, 192_000, 256_000, 320_000, 450_000, 510_000,
];

pub fn build_profile(latency: LatencyProfile, quality: Quality, channels: u32, redundancy: bool) -> StreamProfile {
...
mod tests {
    ...
    fn custom_bounds_are_sane() {
        let (min, max, _) = LatencyProfile::Custom { min_ms: 1, max_ms: 0 }.params();
        assert_eq!(min, 5);
        assert!(max >= min);
    }
}
