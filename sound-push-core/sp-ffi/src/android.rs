...
#[unsafe(no_mangle)]
pub extern "system" fn Java_net_soundpush_engine_NativeContext_nativeInit(env: JNIEnv, _class: JClass, context: JObject) {
    INIT.call_once(|| {
        ...
        unsafe {
            ndk_context::initialize_android_context(vm.get_java_vm_pointer().cast(), global.as_obj().as_raw().cast());
        }
